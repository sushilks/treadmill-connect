...
## 4. Telemetry Packet (Broadcast Data)
The treadmill sends this packet continuously while active (approx. 5Hz).

**Raw Payload (Reassembled):**
`01 02 02 76 05 06 00 00 28 01 00 00 00 00 00 00 F4 01 00 00 ...`

| Offset   | Length | Description        | Scaling / Notes                        |
| :------- | :----- | :----------------- | :------------------------------------- |
| **0-7**  | 8      | **Header**         | Standard Header (Seq, Dest, Src, Cmd?) |
| **8**    | 2      | **Speed** (KPH)    | `Value / 100` (e.g. 296 = 2.96 kph)    |
| **10**   | 2      | **Incline** (%)    | `Value / 100` (e.g. 100 = 1.0%)        |
| **16**   | 4      | **Unknown**        | Often populated but not Speed/Dist.    |
| **27**   | 4      | **Elapsed Time**   | `Value` = Seconds (e.g. 65 = 1m 5s)    |
| **31**   | 4      | **Calories**       | `Value / 97656` (Approximate Factor)   |
| **42**   | 4      | **Total Distance** | `Value / 100` = Meters                 |
| **Last** | 1      | **Checksum**       | Sum Mod 256                            |

*Note: Distance is NOT at offset 16 as typically seen in some other revisions. It is definitely at offset 42 for this model.*
